...
- สวิตช์ใช้ `CupertinoSwitch` พร้อม `activeTrackColor` เป็น accent/semantic color และ `inactiveTrackColor` ชัดเจน (dark `Color(0xFF39393D)`, light `Color(0xFFE9E9EA)` ตาม switch ที่มีอยู่). สถานะใช้แคปซูล `AppRadii.full` และพื้น semantic tint ประมาณ 12%. ชุดตัวเลขเปรียบเทียบใช้ metric grid สองคอลัมน์หรือ 2×3 ให้ label และค่าเทียบกันได้.
