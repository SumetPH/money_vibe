...
Money Vibe คือแอปจัดการการเงินส่วนบุคคลที่เน้นความเรียบง่าย แต่ทรงพลัง รองรับการใช้งานแบบ Offline (SQLite) และ Online (Supabase) พร้อมฟีเจอร์การวิเคราะห์ด้วย AI

## 2. สถาปัตยกรรม (Architecture)
โปรเจกต์นี้ใช้ **Repository Pattern** เพื่อแยก Logic การจัดการข้อมูลออกจาก UI และ Provider

- **Data Layer**: 
    - `DatabaseRepository`: Abstract class ที่กำหนด interface
    - `SqliteRepository`: จัดการข้อมูลในเครื่อง
    - `SupabaseRepository`: จัดการข้อมูลบน Cloud
- **Service Layer**: 
    - `DatabaseManager`: ทำหน้าที่เป็น Mediator ในการสลับระหว่าง SQLite และ Supabase
- **State Management**: 
    - ใช้ **Provider** ในการจัดการ State และเชื่อมต่อกับ `DatabaseManager`
- **Navigation**:
    - ใช้ **GoRouter** ในการจัดการเส้นทาง (Routing)
...
- **No Timezone Conversion**: ห้ามทำการแปลง timezone เมื่อบันทึกหรือดึงข้อมูล เพื่อความเรียบง่ายในการใช้งานข้าม Platform (ยกเว้นกรณีมีเหตุผลจำเป็นพิเศษ)
- **Format**: ใช้มาตรฐาน ISO8601 เมื่อส่งข้อมูลเข้า database
...
### 4.2 การเพิ่มฟีเจอร์ใหม่
1. สร้าง **Model** (หากจำเป็น)
2. เพิ่ม Method ใน `DatabaseRepository` และ Implement ทั้งใน `SqliteRepository` และ `SupabaseRepository`
3. อัปเดต **Provider** ที่เกี่ยวข้อง
4. สร้าง **UI** โดยใช้สีจาก `AppColors` และรองรับ Dark Mode

## 5. กฎเหล็กสำหรับ AI Agent
